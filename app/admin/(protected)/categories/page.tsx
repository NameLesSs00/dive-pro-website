...
export default function AdminCategoriesPage() {
  ...
  const categoriesQuery = useCategories(token, { pageNumber, pageSize, search });
  ...
  const totalCount = Number(pagination?.totalCount ?? categories.length);
  ...
  const currentImagePreview = localImagePreview || getApiAssetUrl(selectedCategory?.imageUrl);
    ...
    <div className="space-y-6">
      <div className="rounded-lg border border-[#D9E4F5] bg-white p-6 shadow-[0_18px_50px_rgba(0,17,58,0.08)] md:p-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-bold text-[#00113A] md:text-4xl">Categories</h1>
            <p className="mt-3 max-w-2xl text-[#5E6675]">
              Manage product categories that will be reused across the store later.
            </p>
          </div>
      ...
      <section className="rounded-lg border border-[#D9E4F5] bg-white p-4 shadow-[0_12px_34px_rgba(0,17,58,0.05)] md:p-6">
        <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <p className="text-sm font-semibold text-[#5E6675]">{totalCount} categories</p>
        </div>
        ...
        {deleteMutation.isError && (
          <div className="mb-5">
            <ApiErrorMessage error={deleteMutation.error} title="Could not delete category" />
          </div>
        )}
        ...
        {categoriesQuery.isLoading ? (
                    ...
                    <tr key={category.id} className="group">
                      <td className="rounded-l-lg border-y border-l border-[#E5ECF8] bg-white px-4 py-3 transition-colors group-hover:bg-[#FBFCFF]">
                        <div className="h-32 w-56 overflow-hidden rounded-lg border border-[#D9E4F5] bg-[#F7FAFF] shadow-sm">
                          {category.imageUrl ? (
                            <img
                              src={getApiAssetUrl(category.imageUrl)}
                              alt={category.name}
          ...
          <div className="rounded-lg bg-[#F7FAFF] px-6 py-12 text-center">
            <h2 className="text-xl font-bold text-[#00113A]">No categories yet</h2>
            <p className="mt-2 text-[#5E6675]">Create the first category from the popup.</p>
          </div>
        )}
      </section>
          ...
          <div className="w-full max-w-md rounded-lg border border-[#D9E4F5] bg-white shadow-[0_24px_80px_rgba(0,17,58,0.18)]">
            <div className="flex items-center justify-between border-b border-[#E5ECF8] px-5 py-4">
              <div>
                ...
                <p className="mt-1 text-sm text-[#5E6675]">
                  {modalMode === 'create' ? 'Add a category name and image.' : 'Update the name or replace the image.'}
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="space-y-4 p-5">
              {showModalError && (
                <ApiErrorMessage
                  error={modalError}
                  title={modalMode === 'create' ? 'Could not create category' : 'Could not update category'}
                />
              )}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                >
                  {isSaving ? 'Saving...' : modalMode === 'create' ? 'Create category' : 'Save changes'}
                </button>
              </div>
            </form>
          </div>
